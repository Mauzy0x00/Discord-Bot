...
module.exports = {
    ...
    .addUserOption(option => option.setName('cuteuser').setDescription('Person to wave to').setRequired(true)),

    async execute(interaction) {
        const query = 'cute wave';
        const limit = 25;
        const mediaFilter = 'minimal';
        const wave = interaction.options.getUser('cuteuser');
        ...
        .setDescription(`${interaction.user.username} is waving at ${wave}`)
        .setImage(gifUrl);
